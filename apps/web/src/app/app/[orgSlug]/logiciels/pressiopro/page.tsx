...
/**
 * PressioPro — Essai pressiométrique Ménard (NF EN ISO 22476-4).
 *
 ...
 * (`apps/web/src/tools-cloned/pressiopro.html`, calcul excisé) chargé en
 ...
 * proxifie `calc:request` vers `runCalc`, jamais de formule/régression
 * côté navigateur).
 *
 * Multi-engine (spécifique PressioPro, cf. ADR 0015 + en-tête `ToolFrame`,
 * même schéma que GEOPLAQUE) : l'outil client regroupe 3 modes internes
 * (onglets), chacun avec son propre moteur de calcul serveur — dépouillement
 * (principal), étalonnage sonde et calibrage de forage (appareillage). Ce ne
 * sont PAS des logiciels séparés (doctrine fidélité interfaces client) : un
 * seul clone, un seul shell, un seul bouton PV sur le DERNIER calcul quel
 * que soit le mode qui l'a produit. Les 3 modes partagent le MÊME gate
 * d'abonnement (`pressiometre`, cf. `software-catalog.ts` — un seul
 * entitlement pour le module) ; `engineAllowlist` autorise `ToolFrame` à
 * relayer TEL QUEL l'engineId déclaré par le clone pour CES 3 slugs
 * uniquement, tout autre engineId étant rejeté avant d'atteindre l'API (cf.
 * `lib/tool-bridge/ToolFrame.tsx`).
 *
 * Historique : la reconstruction React précédente (~3100 lignes, onglets
 * Projet/Log/Mesures/Résultats/Profil + payloads purs dépouillement/
 * étalonnage/calibrage + coupe de sondage + catalogue de sondes/gaines) est
 ...
 * pressiopro » — dernier des 6 logiciels). Les anciens tests de page
 * (`buildPressioProPayload`, `buildEtalonnagePayload`, `buildCalibragePayload`,
 * les graphes de courbe pressiométrique, la coupe de sondage) décrivaient
 * cette implémentation retirée ; ils sont remplacés par
 * `pressiopro-page.test.tsx` (shell + cas multi-engine) + les tests du
 * bridge (`lib/tool-bridge/__tests__/ToolFrame.test.tsx`) + la spec de
 * fidélité clone↔source gelée (à charge de qa-test).
 */
...
const TOOL_ID = 'pressiopro';
// Gate d'abonnement — les 3 modes internes du clone (dépouillement/
// étalonnage/calibrage) sont groupés sous ce seul engineId d'entitlement
// (software-catalog.ts).
...
const ENGINE_ALLOWLIST = ['pressiometre', 'pressio-etalonnage', 'pressio-calibrage'];

// ── Design (accent brique — identique à l'ancienne page / au catalogue GEOFAM) ──
const ACCENT = '#963b28';
const MUTED = '#79706c';
const LINE = '#e6d5cf';
const PANEL = '#fcf9f8';
...
export default function PressioProPage() {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div style={{ fontSize: 19, fontWeight: 700 }}>PressioPro</div>
          <div style={{ fontSize: 12, color: MUTED }}>
            Essai pressiométrique · Pressiomètre Ménard
          </div>
        ...
        >
          ...
          <button
            ...
            style={{
              background: canEmitPv ? ACCENT : '#d0b8b0',
              color: '#fff',
              border: 'none',
              borderRadius: 9,
              padding: '10px 18px',
