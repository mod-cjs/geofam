...
const SOILS: { v: Soil; l: string }[] = [
  { v: 'argile', l: 'Argile / limon' },
  { v: 'sable', l: 'Sable / grave' },
  { v: 'craie', l: 'Craie' },
  { v: 'marne', l: 'Marne' },
  { v: 'roche', l: 'Roche' },
];
 ...
 */
export function downdragMissing(
  fnOn: boolean,
  output: NormalizedCalcOutput | null | undefined,
): boolean {
  if (!fnOn || !output) return false;
  const rows = [...(output.rows ?? []), ...(output.details ?? [])];
  const hasFnRow = rows.some((r) =>
    /frottement n[ée]gatif/i.test(String((r as { label?: unknown }).label ?? '')),
  );
  return !hasFnRow;
}
    ...
    >
      <div
        style={{
          ...
          color: MUTED,
          fontWeight: 600,
          marginBottom: pts.length ? 6 : 0,
        }}
      >
        {pts.length} point{pts.length > 1 ? 's' : ''} interprété
        {pts.length > 1 ? 's' : ''}
        {suspect && (
          <span style={{ color: '#8f2a1f', marginLeft: 8 }}>
            — q_c = 0 détecté : vérifiez que le point est bien la décimale (virgule =
      ...
      {pts.length > 0 && (
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          ...
          <tbody>
            {pts.map((p, i) => (
              <tr key={i}>
                <td style={{ padding: '2px 4px', color: MUTED, fontSize: 11 }}>
                  {i + 1}
                </td>
                ...
                <td
                  style={{
                    padding: '2px 4px',
                    color: p.qc === 0 ? '#8f2a1f' : INK,
                    fontWeight: p.qc === 0 ? 700 : 400,
                  }}
                >
                  {p.qc}
                </td>
...
export default function CasagrandePage() {
  ...
  const buildPayload = useCallback(
    () => buildCasaPayload(buildFormState()),
    [buildFormState],
  );
      ...
      setCalcError(
        e?.reason === 'EXPIRED'
          ...
          : e?.reason === 'QUOTA'
            ...
            : e?.reason === 'MODULE_NOT_IN_PACK'
              ? "Le module CASAGRANDE n'est pas inclus dans votre abonnement."
  ...
  const upLayer = (i: number, k: keyof LayerRow, v: string) =>
    setLayers((p) => p.map((r, j) => (j === i ? { ...r, [k]: v } : r)));
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 12, color: MUTED }}>
            Fondations profondes — pieux · NF P 94-262 / Eurocode 7 · capacité portante
          </div>
        </div>
        <div
        ...
        >
          ...
          <button
            ...
            style={{
              background: calcDisabled ? '#b7c2bd' : ACCENT,
              color: '#fff',
              border: 'none',
              borderRadius: 9,
              padding: '10px 18px',
            ...
            <div style={grid3}>
              <div>
                ...
                >
                  ...
                  <option value="quel">Quelconque</option>
                </select>
              </div>
              {section === 'quel' ? (
                <>
                  ...
                  <div>
                    <label style={lbl}>Périmètre du fût P (m)</label>
                    <input
                      style={inp}
                      value={gP}
                ...
                <>
                  <div>
                    <label style={lbl}>
                      {section === 'circ' ? 'Diamètre B (m)' : 'Côté / largeur B (m)'}
                    </label>
                    <input
                      style={inp}
                      value={gB}
              ...
              <div>
                <label style={lbl}>Fiche D (m)</label>
                <input style={inp} value={gD} onChange={(e) => setGD(e.target.value)} />
              </div>
              <div>
                <label style={lbl}>Cote de départ z₀ (m)</label>
                <input style={inp} value={gz0} onChange={(e) => setGz0(e.target.value)} />
              </div>
            </div>
          </div>
          <div style={card}>
            ...
            <div style={grid3}>
              <div>
                ...
                <select
                  style={inp}
                  value={meth}
                  onChange={(e) => setMeth(e.target.value as Meth)}
                >
                  <option value="pmt">Pressiomètre (pl*)</option>
                  <option value="cpt">Pénétromètre (qc)</option>
                  <option value="cphi">Labo (c, φ)</option>
                </select>
              </div>
              <div>
                <label style={lbl}>Approche de calcul</label>
                <select
                  style={inp}
                  value={da}
                  onChange={(e) => setDa(e.target.value as Da)}
                >
                  <option value="da1">DA1 (A1+M1+R1 / A2+M1+R4)</option>
                  <option value="da2">DA2 — NF P 94-262 (A1+M1+R2)</option>
                  <option value="da3">DA3 (M2+R3)</option>
                </select>
              </div>
              <div>
                <label style={lbl}>Sens de sollicitation</label>
                <select
                  style={inp}
                  value={sens}
              ...
              <div>
                <label style={lbl}>Charge permanente G (kN)</label>
                <input style={inp} value={cG} onChange={(e) => setCG(e.target.value)} />
              </div>
              <div>
                <label style={lbl}>Charge variable Q (kN)</label>
                <input style={inp} value={cQ} onChange={(e) => setCQ(e.target.value)} />
              </div>
              <div>
                <label style={lbl}>Essais renforcés</label>
                <select
                  style={inp}
                  value={essais}
                  onChange={(e) => setEssais(e.target.value as 'oui' | 'non')}
                >
        ...
        <>
          <div style={card}>
            ...
            <label style={{ ...lbl, display: 'flex', gap: 7, alignItems: 'center' }}>
              <input
                type="checkbox"
                checked={fnOn}
                onChange={(e) => setFnOn(e.target.checked)}
              />{' '}
              Calculer le frottement négatif (downdrag)
            </label>
            <div
              style={{ fontSize: 10.5, color: MUTED, fontStyle: 'italic', marginTop: 4 }}
            >
              Reporté à titre indicatif — non intégré au verdict de portance ; action
              permanente à ajouter à la charge en tête (NF P 94-262, Annexe H).
            </div>
            {fnOn && (
              <>
                <div style={{ marginTop: 12, marginBottom: 10 }}>
                  ...
                  <select
                    style={inp}
                    value={fnMode}
                    onChange={(e) => setFnMode(e.target.value as 'auto' | 'impose')}
                  >
                ...
                {fnMode === 'auto' ? (
                  <div style={grid3}>
                    <div>
                      <label style={lbl}>Tassement en surface s₀ (mm)</label>
                      <input
                        style={inp}
                        value={fnS0}
                        onChange={(e) => setFnS0(e.target.value)}
                        placeholder="90"
                      />
                    </div>
                    <div>
                      <label style={lbl}>Profondeur compressible H_c (m)</label>
                      <input
                        style={inp}
                        value={fnHc}
                        onChange={(e) => setFnHc(e.target.value)}
                        placeholder="9.0"
                      />
                    </div>
                  </div>
                  ...
                  <div style={grid3}>
                    <div>
                      <label style={lbl}>Frottement négatif de (m)</label>
                      <input
                        style={inp}
                        value={fnZt}
                        onChange={(e) => setFnZt(e.target.value)}
                        placeholder="0.0"
                      />
                    </div>
                    <div>
                      <label style={lbl}>… jusqu&apos;à (m)</label>
                      <input
                        style={inp}
                        value={fnZb}
                        onChange={(e) => setFnZb(e.target.value)}
                        placeholder="9.0"
                      />
                    </div>
                  </div>
          ...
          {fnOn && (
            <div style={card}>
              ...
              <div style={grid3}>
                <div>
                  <label style={lbl}>Charge structurelle en tête Q (kN)</label>
                  <input
                    style={inp}
                    value={fnQ}
                ...
                <div>
                  <label style={lbl}>Terme K·tanδ (frottement négatif)</label>
                  <input
                    style={inp}
                    value={fnKtd}
                    onChange={(e) => setFnKtd(e.target.value)}
                    placeholder="0.30"
                  />
                </div>
              </div>
              ...
              >
                Frottement négatif plafonné à q_sn = K·tanδ·σ′_v (Combarieu) au-dessus du
                point neutre ; en dessous, le frottement positif reprend la charge.
              </div>
            </div>
          )}
        </>
        ...
        <>
          <div style={card}>
            ...
            <div style={{ ...grid3, marginBottom: 12 }}>
              <div>
                <label style={lbl}>Nappe (m/TN)</label>
                <input
                  style={inp}
                  value={nappe}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
              ...
              <tbody>
                {layers.map((r, i) => (
                  <tr key={i}>
                    <td style={{ padding: 2, minWidth: 110 }}>
                      <select
                        style={inp}
                        value={r.soil}
                        onChange={(e) => upLayer(i, 'soil', e.target.value)}
                      >
                        {SOILS.map((s) => (
                    ...
                    {(['th', 'pl', 'em', 'qc', 'c', 'phi', 'gamma'] as const).map((k) => (
                      <td key={k} style={{ padding: 2 }}>
                        <input
                          style={inp}
                          value={r[k]}
                          onChange={(e) => upLayer(i, k, e.target.value)}
                          placeholder={k === 'th' ? '' : '—'}
                        />
                      </td>
                    ))}
          ...
          {meth === 'cpt' && (
            <div style={card}>
              ...
              <div style={{ ...grid3, marginBottom: 10 }}>
                <div>
                  <label style={lbl}>Pas de profondeur (m)</label>
                  <input
                    style={inp}
                    value={cptStep}
                    onChange={(e) => setCptStep(e.target.value)}
                    placeholder="0.20"
                  />
                </div>
              </div>
              <label style={lbl}>
                Sondage collé — une ligne « z(m) q_c(MPa) » par profondeur · point =
                décimale, espace/virgule/point-virgule = séparateur de colonne
              </label>
              <textarea
                value={cptPaste}
              ...
              >
                q_ce et le frottement sont calculés sur ce pénétrogramme. Laissé vide, il
                est régénéré à partir des q_c des couches. Utilisez le{' '}
                <strong>point</strong> comme séparateur décimal ; la virgule est traitée
                comme un séparateur de colonne.
              </div>
            </div>
          )}
        </>
      ...
      {tab === 'options' && (
        <>
          <div style={card}>
            ...
            <div style={grid3}>
              <div>
                <label style={lbl}>Nombre de files n</label>
                <input
                  style={inp}
                  value={grpN}
                  onChange={(e) => setGrpN(e.target.value)}
                />
              </div>
              <div>
                <label style={lbl}>Pieux par file m</label>
                <input
                  style={inp}
                  value={grpM}
                  onChange={(e) => setGrpM(e.target.value)}
                />
              </div>
              <div>
                <label style={lbl}>Entraxe S (m) · 0 = isolé</label>
                <input
                  style={inp}
                  value={grpS}
                  onChange={(e) => setGrpS(e.target.value)}
                />
              </div>
            </div>
          </div>
          <div style={card}>
            ...
            <div style={grid3}>
              <div>
                <label style={lbl}>Nb. de profils de sondage (N)</label>
                <input
                  style={inp}
                  value={nprofil}
                  onChange={(e) => setNprofil(e.target.value)}
                />
              </div>
              <div>
                ...
                <div style={{ fontSize: 10.5, color: MUTED, marginTop: 3 }}>
                  Facteurs de corrélation ξ (E.2.1) — bornée 100–2500 m².
                </div>
              </div>
              <div>
                <label style={lbl}>Redistribution</label>
                <select
                  style={inp}
                  value={redis}
                  onChange={(e) => setRedis(e.target.value as 'oui' | 'non')}
                >
                  <option value="non">Non</option>
                  <option value="oui">Oui</option>
                </select>
          ...
          <div style={card}>
            ...
            <label style={{ ...lbl, display: 'flex', gap: 7, alignItems: 'center' }}>
              <input
                type="checkbox"
                checked={betonOn}
                onChange={(e) => setBetonOn(e.target.checked)}
              />{' '}
              Vérification structurelle du fût
            </label>
            {betonOn && (
              <div style={{ ...grid3, marginTop: 8 }}>
                <div>
                  <label style={lbl}>fck (MPa)</label>
                  <input
                    style={inp}
                    value={fck}
                ...
                <div>
                  <label style={lbl}>Armature</label>
                  <select
                    style={inp}
                    value={arm}
                    onChange={(e) => setArm(e.target.value as 'arme' | 'nonarme')}
                  >
                    <option value="arme">Armé</option>
                    <option value="nonarme">Non armé</option>
                  </select>
                </div>
                <div>
                  ...
                  <select
                    style={inp}
                    value={k3}
                    onChange={(e) => setK3(e.target.value as '1.0' | '1.2')}
                  >
          ...
          <div style={card}>
            ...
            <div style={{ fontSize: 11.5, color: MUTED }}>
              γ_G = {DEFAULT_COEFFS.k_gG} · γ_Q = {DEFAULT_COEFFS.k_gQ} · γ_b ={' '}
              {DEFAULT_COEFFS.k_gb} · γ_s = {DEFAULT_COEFFS.k_gs} · γ_s;t ={' '}
              {DEFAULT_COEFFS.k_gst} · ξ fluage compression = {DEFAULT_COEFFS.cr_car}.
            </div>
            <div
              style={{ fontSize: 10.5, color: MUTED, fontStyle: 'italic', marginTop: 4 }}
            >
              Coefficients partiels <strong>fixés côté serveur</strong> (valeurs
              réglementaires NF P94-262 DA2), bloc informatif non modifiable : toute
              valeur non normative envoyée à l&apos;API est rejetée (intégrité du
              verdict/PV).
            </div>
          </div>
        </>
          ...
          {isCalcError ? (
            ...
            <>
                  ...
                  background:
                    output.verdict === 'PASS'
                      ...
                      : output.verdict === 'FAIL'
                        ? '#fbeae7'
                        : '#f4edd8',
                  border: `1px solid ${output.verdict === 'PASS' ? '#a9d0b3' : output.verdict === 'FAIL' ? '#e0b3aa' : '#e6cf9c'}`,
                }}
              >
                <b
                  style={{
                ...
                >
                  {output.verdict === 'PASS'
                    ? 'Pieu vérifié — critères EC7 satisfaits'
                    : output.verdict === 'FAIL'
                      ? 'Pieu non vérifié — reprise nécessaire'
                      : 'Résultats de vérification'}
                </b>
              </div>
              {downdragMissing(fnOn, output) && (
                <div
                  data-testid="fn-missing"
                  role="alert"
                  style={{
                    padding: '10px 14px',
                    borderRadius: 10,
                ...
                >
                  Frottement négatif demandé mais non calculé : vérifiez les paramètres de
                  l&apos;onglet « Frottement négatif » (charge Q, K·tanδ, zone
                  d&apos;action). Aucun résultat de downdrag n&apos;est disponible pour ce
                  calcul.
                </div>
              )}
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr>
                    {['Grandeur', 'Valeur', 'Unité', 'Statut'].map((h) => (
                      <th
                        key={h}
                        style={{
                          ...th,
                        ...
                        {row.status && (
                          <span
                            style={{
                              ...
                              padding: '3px 8px',
                              borderRadius: 6,
                              background: row.status === 'ok' ? '#e4efe6' : '#fbeae7',
                              color: row.status === 'ok' ? '#2e7d4f' : '#b23a2e',
                            }}
                          >
              ...
              >
                Intermédiaires et contexte exposés ci-dessus ; seuls les facteurs de
                portance et coefficients de calage restent côté serveur (DoD §8).
              </div>
              <div style={{ marginTop: 16 }}>
                {pvResult ? (
                  <>
