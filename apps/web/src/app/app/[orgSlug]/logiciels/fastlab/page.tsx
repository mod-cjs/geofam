...
/**
 * FASTLAB — Classification GTR des sols / essais de laboratoire (NF P11-300).
 *
 ...
 * (`apps/web/src/tools-cloned/fastlab.html`, calcul excisé) chargé en
 ...
 * classification côté navigateur).
 *
 * Un seul engineId ('labo') — pas de multi-engine ici (contrairement à
 * GEOPLAQUE) : pas d'`engineAllowlist`.
 *
 * Particularité FASTLAB (ADR 0015 §Conséquences) : le clone envoie ses
 * `calc:request` en rafale débouncée (~300 ms, POST unique portant l'état
 * complet du formulaire) à chaque modification de saisie — plusieurs
 * `calcResultId` peuvent donc arriver coup sur coup pendant une session.
 * `onCalcResultId` (prop de `ToolFrame`) est un simple `setState` : le
 * dernier appel gagne toujours, donc le bouton PV suit nativement le
 * DERNIER calcul reçu, sans logique supplémentaire ici.
 *
 * Historique : la reconstruction React précédente (~2194 lignes, formulaire
 * à onglets essais d'identification/labo + classification affichée en
 * dur + assistants « points à vérifier »/famille R dupliqués) est retirée
 * avec cette généralisation du pilote terzaghi (cf. ADR 0015 §Décision —
 * « pilote terzaghi, puis geoplaque, fastlab, casagrande, pressiopro »). Les
 * anciens tests de page (`buildFastlabPayload`, `EXTRA_SECTIONS`,
 * `ExtraView`, encarts « points à vérifier »/« assistant famille R »)
 * décrivaient cette implémentation retirée ; ils sont remplacés par
 * `fastlab-page.test.tsx` (shell) + les tests du bridge
...
const TOOL_ID = 'fastlab';
const ENGINE_ID = 'labo';

// ── Design (accent olive — identique à l'ancienne page / au catalogue GEOFAM) ──
const ACCENT = '#6b7a2e';
const MUTED = '#71767a';
const LINE = '#dde0d0';
const PANEL = '#fbfcf8';
...
export default function FastlabPage() {
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div style={{ fontSize: 19, fontWeight: 700 }}>FASTLAB</div>
          <div style={{ fontSize: 12, color: MUTED }}>
            Classification laboratoire · GTR · NF P11-300
          </div>
        ...
        >
          ...
          <button
            ...
            style={{
              background: canEmitPv ? ACCENT : '#b7bfad',
              color: '#fff',
              border: 'none',
              borderRadius: 9,
              padding: '10px 18px',
