...
const GROUPS: Array<{ domain: Domain; label: string; ids: string[] }> = [
  { domain: 'road', label: 'Chaussées', ids: ['burmister'] },
  {
    domain: 'foundation',
    label: 'Fondations',
    ids: ['terzaghi', 'casagrande', 'geoplaque'],
  },
    ...
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', width: '100%' }}>
      <div style={{ marginBottom: 28 }}>
        ...
        >
          ...
          {entitlements && (
            <span>
              {' '}
              Pack actif : <strong>{entitlements.pack}</strong>.
            </span>
          )}
        </p>
      </div>
      ...
      {loading && (
        <div aria-busy="true" aria-label="Chargement de la bibliothèque">
          {[1, 2, 3].map((i) => (
            <Skeleton key={i} variant="card-projet" style={{ marginBottom: 8 }} />
          ))}
        </div>
      )}
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          >
                            {desc.fields.filter((f) => f.type !== 'section').length}{' '}
                            champs de saisie
                          </div>
                        </div>
                      </div>
