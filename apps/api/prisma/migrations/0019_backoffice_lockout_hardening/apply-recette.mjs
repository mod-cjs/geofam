// =====================================================================
//  Application MANUELLE de la migration 0019 en recette (node pg).
//
//  Usage :  DATABASE_URL="postgres://..." node apply-recette.mjs
//
//  A jouer par un HUMAIN, apres 0018, sous le compte de deploiement (membre de
//  roadsen_auth + CREATE sur public ; PAS besoin de BYPASSRLS). Ce script :
//    - pose SET lock_timeout (echec franc plutot que hang sur une ligne users verrouillee) ;
//    - ouvre UNE transaction (BEGIN) : soit tout passe, soit rien (0019 est reversible) ;
//    - applique migration.sql (CREATE OR REPLACE des 2 fonctions — idempotent) ;
...
//    - COMMIT.
//  Rollback documente : down.sql (restaure les corps 0015/0007).
//
...
const NAME = '0019_backoffice_lockout_hardening';
...
try {
  ...
  console.log(`0019 applique + enregistre (checksum ${checksum.slice(0, 12)}...).`);
} catch (err) {
  try {
  ...
  console.error('Echec 0019 (rollback effectue) :', err.message ?? err);
  process.exit(1);
} finally {
  await client.end();
}
