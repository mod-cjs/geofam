...
describe('POST /calc/pieux (e2e)', () => {
  ...
  it('un cas nominal renvoie la MEME sortie que le module (client<->serveur)', async () => {
    ...
    // Aucun intermediaire (terme de pointe, facteurs de portance, frottement, courbe
    // de tassement) dans la reponse.
    expect(JSON.stringify(res.body)).not.toMatch(
      /"(qb|ple|qce|kfac|kmax|Def|fric|qbDetail|settle|xi3|comb|baseLayer|layers)"\s*:/,
    );
  });
  ...
  it('un cas hors-domaine (D <= z0) renvoie un resultat borne, sans intermediaire', async () => {
    ...
    expect(JSON.stringify(res.body)).not.toMatch(
      /"(qb|ple|qce|kfac|kmax|Def|fric|qbDetail|settle)"\s*:/,
    );
  });
});
