/**
 * EQUIVALENCE-PORTAGE calibrage : module TS extrait <-> HTML d'origine (jsdom).
 *
 ...
 * `calibRows`, on hooke `renderCalibResult` pour capturer l'objet `e` — voir
 * loadOriginalCompute) ET le module TS, et on compare les resultats BRUTS `e`
 * (a_calib/R²/RMS/c0/c1/c2 + intermediaires pts/residuals) a une tolerance de portage
 * SERREE (rel 1e-9). Le harnais golden @roadsen/shared est l'arbitre (anti auto-reference).
 *
...
import {
  calibrageRegistrySha256,
  calibrageSourceAvailable,
  calibrageSourceSha256,
  jsonRoundTrip,
  loadOriginalCompute,
  sanitizeResult,
...
const MODULE_UNDER_TEST = 'pressio-calibrage';
...
describe('calibrage — equivalence-portage module <-> HTML d origine (@science-unsigned)', () => {
  if (!SOURCE_OK) {
    const msg =
  ...
  const cmpFixtures = PRESSIO_CALIBRAGE_FIXTURES.filter((f) => !f.horsDomaine);
  const horsDomaine = PRESSIO_CALIBRAGE_FIXTURES.filter((f) => f.horsDomaine);
  ...
  it('le SHA-256 de la source testee == sha256 scelle au registre (pas un autre moteur)', () => {
    expect(calibrageSourceSha256()).toBe(calibrageRegistrySha256());
  });
  ...
  for (const fx of cmpFixtures) {
    it(`[${fx.id}] module == origine (rel ${PORTAGE_TOLERANCE.rel}) — ${fx.description}`, () => {
      ...
      const result = runGoldenCase(testCase, MODULE_UNDER_TEST, (inputs: unknown) => {
        const inp = inputs as (typeof PRESSIO_CALIBRAGE_FIXTURES)[number]['input'];
        return sanitizeResult(jsonRoundTrip(computeCalibrage(inp)));
      });
      if (!result.equal) {
  ...
  for (const fx of horsDomaine) {
    it(`[${fx.id}] resultat IDENTIQUE origine/module — ${fx.description}`, () => {
      ...
      const mod = sanitizeResult(jsonRoundTrip(computeCalibrage(fx.input)));
