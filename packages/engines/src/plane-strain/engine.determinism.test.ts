/**
 * DETERMINISME & PURETE (DOM-free) du module deformations planes.
 *
 ...
 *     instable (`for..in`). Le `solvePlaneStrain` d'origine lisait la globale
 *     `state.layers` ; cela a ete REMPLACE par un etat injecte a l'extraction.
 *
...
describe('deformations planes — determinisme (meme entree -> meme sortie x100, egalite stricte)', () => {
  for (const fx of PLANE_STRAIN_FIXTURES) {
    it(`[${fx.id}] sortie identique sur 100 appels`, () => {
      const ref = JSON.stringify(computePlaneStrain(fx.input, fx.input.opts));
      for (let i = 0; i < 100; i++) {
        expect(JSON.stringify(computePlaneStrain(fx.input, fx.input.opts))).toBe(ref);
      }
    });
  }

  it('ne MUTE pas l entree (2 appels sur le MEME objet -> meme resultat)', () => {
    const fx = PLANE_STRAIN_FIXTURES.find((f) => f.id === 'bande-lineiques-multiples');
    expect(fx).toBeDefined();
    if (!fx) return;
    const first = JSON.stringify(computePlaneStrain(fx.input, fx.input.opts));
    const second = JSON.stringify(computePlaneStrain(fx.input, fx.input.opts));
    expect(second).toBe(first);
  });

  it('SMOKE : un cas nominal produit wMax/mMax/sumReact finis', () => {
    const fx = PLANE_STRAIN_FIXTURES.find((f) => f.id === 'bande-lineique-centree');
    expect(fx).toBeDefined();
    if (!fx) return;
    const R = computePlaneStrain(fx.input, fx.input.opts) as Record<string, unknown>;
    expect(R.err).toBeUndefined();
    expect(Number.isFinite(R.wMax as number)).toBe(true);
    expect(Number.isFinite(R.mMax as number)).toBe(true);
    expect(Number.isFinite(R.sumReact as number)).toBe(true);
  });
});

describe('deformations planes — gardes du moteur exercees AU NIVEAU MOTEUR (avant schema)', () => {
  it('aucune couche -> erreur bornee « au moins une couche de sol »', () => {
    const r = computePlaneStrain(
      { layers: [] },
      { Bw: 6, E: 32000, nu: 0.2, e: 0.4, q: 50 },
...
describe('deformations planes — purete DOM/IO (le module est DOM-free)', () => {
