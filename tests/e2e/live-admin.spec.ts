 ...
 * /admin (fix : un compte sans org part au back-office) ; (2) la console liste les orgs
 * réelles ; (3) la page DÉTAIL d'une org se charge (fix : plus de crash SSR dû à un
 * gestionnaire onMouseOver dans un Server Component).
 *
...
test.describe('LIVE — Console back-office SUPERADMIN (Vercel ↔ Render)', () => {
  ...
  test('login superadmin -> REDIRECTION AUTO vers /admin + orgs réelles', async ({ page }) => {
    await submitLogin(page);
    // FIX #1 : redirection automatique vers le back-office (aucune navigation manuelle).
    await page.waitForURL(/\/admin(\/orgs)?\/?$/, { timeout: 60_000 });
